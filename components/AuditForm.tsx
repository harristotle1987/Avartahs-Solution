...
const AuditForm: React.FC<AuditFormProps> = ({ onAuditComplete }) => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      cta_source: 'system handshake v4'
    };
    ...
    try {
      ...
      if (publicKey && publicKey !== 'undefined') {
        await emailjs.send(
          ...
          import.meta.env.VITE_EMAILJS_TEMPLATE_ID || 'template audit',
          {
  ...
  if (isSuccess) {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-6 sm:p-8 md:p-12 bg-white dark:bg-[#0f172a] border-2 border-midnight dark:border-white/10 rounded-[2rem] sm:rounded-[3rem] shadow-highlight relative overflow-hidden text-center">
        ...
        <h3 className="text-xl md:text-2xl font-black text-midnight dark:text-white uppercase tracking-tighter mb-4">[ Data Received ]</h3>
  ...
  return (
    <div className="bg-white dark:bg-[#0f172a] border-2 border-midnight dark:border-white/10 rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-8 md:p-10 shadow-premium relative overflow-hidden">
      <div className="flex items-center justify-between mb-8">
        <div className="flex flex-col">
          <div className="text-[10px] font-black uppercase tracking-[0.4em] text-sunset mb-1">{current.directive}</div>
          <div className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-600">Audit Pipeline</div>
        </div>
