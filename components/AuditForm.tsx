...
const AuditForm: React.FC<AuditFormProps> = ({ onAuditComplete }) => {
  ...
  if (isSuccess) {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-8 md:p-12 bg-white dark:bg-[#0f172a] border-2 border-slate-200 dark:border-white/10 rounded-[3rem] shadow-highlight relative overflow-hidden text-center">
  ...
  return (
    <div className="bg-white dark:bg-[#0f172a] border-2 border-slate-200 dark:border-white/10 rounded-[2.5rem] p-6 md:p-10 shadow-premium relative overflow-hidden">
          ...
          {current.type === 'segmented' ? (
            <div className="grid grid-cols-1 gap-3">
              {[{ label: '[TIER ALPHA] — $300 - $1,000', value: 'ALPHA' }, { label: '[TIER BETA] — $1,000 - $3,000', value: 'BETA' }, { label: '[TIER GAMMA] — $3,000 - $5,000+', value: 'GAMMA' }].map(tier => (
                <button key={tier.value} disabled={isProcessing} onClick={() => { setFormData({...formData, budget: tier.value}); if(!isProcessing) handleNext(); }} className={`w-full py-5 border-2 rounded-2xl font-black text-[10px] uppercase tracking-widest text-left px-6 flex justify-between items-center ${formData.budget === tier.value ? 'border-sunset bg-sunset/5 text-sunset' : 'border-slate-100 dark:border-white/5 text-midnight dark:text-white hover:border-slate-300 dark:hover:border-white/20'}`}>
                  {tier.label} <ChevronRight size={16} />
                </button>
              ))}
            </div>
          ) : (
            <input autoFocus disabled={isProcessing} type={current.field === 'email' ? 'email' : 'text'} placeholder={current.placeholder} value={formData[current.field as keyof typeof formData]} onChange={(e) => setFormData({...formData, [current.field]: e.target.value})} onKeyDown={(e) => e.key === 'Enter' && current.valid && handleNext()} className="w-full bg-slate-50 dark:bg-white/[0.02] border-b-2 border-slate-200 dark:border-white/10 focus:border-sunset py-3 text-lg font-black text-midnight dark:text-white outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600" />
          )}
          {current.type !== 'segmented' && (
            <button disabled={!current.valid || isProcessing} onClick={handleNext} className="w-full py-5 bg-midnight dark:bg-white text-white dark:text-midnight rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-2 shadow-md hover:bg-slate-800 dark:hover:bg-slate-200 transition-colors">
